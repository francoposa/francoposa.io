...
Pods are a relatively low-level abstraction for sets of containers that can be scaled up and down,
but they are relatively inconvenient to interact with and manage manually.
...
And while were able to reference the Deployment name to tail the logs or choose a pod to port-forward,
it is nothing more than a slightly-more-convenient-but-still-manual way of using selectors to choose Pods for an operation.

#### The Service Resource

A  Service resource solves for the networking limitations of Pods.
Our present concerns are:
1. the internal DNS address assigned to a Pod is based on its name, which only lasts as long as the Pod itself
