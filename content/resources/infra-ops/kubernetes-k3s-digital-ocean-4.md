...
##### 2.3.1 View the Created Resources
...
If at any point we need more detailed information, we can use `kubectl describe` for a more detailed description -
this is where we can find the labels and other metdata defined for the resource, which other resources it relates to,
and pretty much any other information Kubernetes has about the resource.
...
##### 2.3.3 Port-Forward to a Pod (The Fun Part!)
...
by the deployment in order to ignore using the pod names.

Remember that we named our container port `web`, so we do not have to remember which port it is.
...
Now take a moment to celebrate!
